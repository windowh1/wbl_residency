    ...
    try:
        ...
        for item in existing_custom_skills:
            ...
            if skill_name in remaining_skills:
                if force_update:
                    print(f"Deleting existing version of '{skill_name.split('/')[-1]}' skill for update...")
        ...
        for skill_name in remaining_skills:
            ...
            print(f"Uploading '{skill_name.split('/')[-1]}' skill...")     
...
if __name__ == "__main__":
    ...
    client = Anthropic(api_key=claude_conf["api_key"])
    
    # List existing custom skills
    skills = list_skills(client)
    print(f"Existing custom skills: {len(skills)}")
    for skill in skills:
        display_title = skill.get('display_title', 'unknown')
        print(f"  - {display_title}")    

    # Delete all custom skills
    if skills:
        print("\nDeleting all custom skills...")   
        for skill in skills:
            display_title = skill.get('display_title', 'unknown')
            print(f"  - {display_title}")    
            delete_skill(client, skill["skill_id"])

        skills = list_skills(client)
        print(f"\nRemaining custom skills: {len(skills)}")
