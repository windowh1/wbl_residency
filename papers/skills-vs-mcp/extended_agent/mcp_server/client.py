...
class MCPClient:
    ...
    def __init__(
        ...
        Args:
            ...
            url: HTTP URL for streamable HTTP transport (e.g., "http://localhost:8081/mcp")
        """
        self.name = name
        ...
        self.transport_type = "http" if url else "stdio"
            ...
            if self.transport_type == "http":
                ...
                async with streamablehttp_client(self.url) as (read, write, get_session_id):
                    async with ClientSession(read, write) as session:
                        ...
                        print(f"[{self.name}] Successfully connected to HTTP server. Added {len(self.tools)} tools.")
                ...
                async with stdio_client(params) as (stdio, write):
                    ...
                    async with ClientSession(stdio, write) as session:
                        ...
                        print(f"[{self.name}] Successfully connected to stdio server. Added {len(self.tools)} tools.")
    ...
    async def disconnect(self):
        ...
        """
        if not self._task:
            print(f"[{self.name}] Server connection already closed.")
            return
        ...
        try:
            ...
            print(f"[{self.name}] Server connection closed.")
        finally:
            self._task = None
            self.session = None
